...
> **Implementation status:** This plan has been implemented. The current crate layout, feature flags, and runtime behavior are documented in `crates/ocs_doc_api/README.md`, `crates/ocs_doc_api/doc/ARCHITECTURE.md`, and `crates/ocs_doc_api/doc/IMPLEMENTATION.md`. Some details below (e.g. default runtime deps, wire format) reflect early planning and have evolved; see the living documentation for the authoritative state.
...
| Decision | Choice |
|---|---|
| Object-model scope | All first-level `EntityType` variants plus shared value types. Document-level roots (`CadDocument`, tables, header, objects) are intentionally excluded to avoid `serde-reflection` failures and short-name collisions. |
...
1. Trace selected `acadrust` entity and shared value roots with `serde-reflection`.
2. Provide a concrete sample for every first-level `EntityType` variant.
3. Back-fill any entity shape that `serde-reflection` cannot finish (recursive ACIS data, unresolved optional boxes, etc.) with a JSON-derived registry entry.
